...
public class OrderList121901 {
	...
	String[] head={"奶品代码","奶品名称","单位","单价","数量","金额"};
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OrderList121901 window = new OrderList121901();
	...
	private void initialize() {
		...
		frame.setBounds(20, 20, 1187, 700);
		...
		tableModel.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {					
				...
				if(column==5) //更改金额小计时不触发事件，金额小计栏的索引值：6，这行代码很重要，不加狂报错
				{
					Float total2=(float) 0.00;
					Float total22=(float) 0.00;
					for (int i = 0; i < table.getRowCount(); i++) {
						total2=total2+ Float.valueOf( model.getValueAt(i, 5).toString());
						total22=total22+ Float.valueOf( model.getValueAt(i, 4).toString());
					}
					...
					if (type == TableModelEvent.UPDATE) {
						...
						Object shop_price=model.getValueAt(row,3);

//						System.out.println("date="+data+",price="+shop_price);				
						
						String sa=multiply(data.toString(),shop_price.toString());
						Float fa=(float)(Math.round(Float.valueOf(sa) * 100))/100;
						
//						System.out.println("转换后"+fa);
						
						table.getModel().setValueAt(fa, row, 5);
				}
				
					
//					System.out.println("总行数="+table.getRowCount());
...
//		frame.getContentPane().add(table);
		...
		scrollPane.setBounds(425, 129, 715, 428);
		...
		tableproduct.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 2) {
					...
					Float  a3=(Float) tableproduct.getModel().getValueAt(sr, 3);
					...
					for (int i = 0; i < rows; i++) {
						totalamout=totalamout+ Float.valueOf(table.getModel().getValueAt(i, 5).toString());
						totalnum=totalnum+ Float.valueOf(table.getModel().getValueAt(i, 4).toString());
					}
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if("".equals(custname.getText())||"null".equals(custname.getText())) {
			    	...
			    	for (int i = 0; i < rows; i++) {		    		
//						j.insertOrderDetel(orderid, orderdetailid.concat(String.valueOf(i)), table.getModel().getValueAt(i, 0).toString(), table.getModel().getValueAt(i, 1).toString(),Float.valueOf(table.getModel().getValueAt(i, 4).toString()), Float.valueOf(table.getModel().getValueAt(i, 3).toString()), Float.valueOf(table.getModel().getValueAt(i, 5).toString()),table.getModel().getValueAt(i, 2).toString());
					}
					//关闭数据连接和关闭窗口
					j.closeOrderJdbc();
//					JOptionPane.showMessageDialog(null, "订单插入完成！");
		...
		button_2.setBounds(1047, 594, 93, 23);
    ...
    protected void swingPrint(String dh) {
    	...
    	tt.printTable(tableModel, "送货单",this.custname.getText(),dh,dateString22,this.sendname.getText(),this.total_amout.getText(),"北京百和乐商贸有限公司");
	}
...
//		}
	...
	protected void uu(String a1, String a2, String dw, Float a3) {
		// TODO Auto-generated method stub
